...
// Note: autoGeneratedBlogs are hidden SEO pages - not shown in blog listing
// They are only accessible via direct URL (for Google search traffic)

// Curated visible blogs for the main website (limit to best ones)
...
const Blog = () => {
  ...
  // Fetch blog posts from API - only visible ones for main site
  useEffect(() => {
    const fetchBlogPosts = async () => {
      try {
        // Try to get visible blogs from API first
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/blogs/visible`);
        if (response.ok) {
          const apiBlogs = await response.json();
          // Merge with static visible blogs (seoBlogPosts and newBlogPosts are curated)
          ...
          // Combine and limit total visible blogs
          ...
          // Fallback to curated static posts only
        ...
        // Fallback to curated static posts
        const curatedPosts = [...newBlogPosts, ...seoBlogPosts.slice(0, 6), ...staticBlogPosts];
        setBlogPosts(curatedPosts.slice(0, VISIBLE_BLOG_LIMIT));
      } finally {
        setLoading(false);
